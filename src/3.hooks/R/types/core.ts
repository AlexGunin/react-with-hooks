...
export interface GlobalState {
    ...
    /** Флаг, указывающий на то, что запущен следующий рендер
     * Нужен для ситуаций, когда делаем несколько SetState в разных компонентах,
     * т.к. в такой ситуации важно корректно определить wipRoot, то есть корень поддерева в котором будет перерендер
     * Если определим его неправильно, то либо какая-то часть дерева не обновится (хотя должна была),
     * либо перерендерим то, что не должны были
     * */
    pending: boolean;
}
